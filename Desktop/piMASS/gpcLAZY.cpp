...
gpcLAZY* gpcLAZY::tree_add( U4 id, U4& n )
{
	...
	if( !this )
	{
		gpcLAZY* p_this = lazy_add( NULL, sizeof(U44), s, 8 );
		if( !p_this )
			return NULL;

		n = 1;
		((U44*)p_this->p_alloc)->null().x = id;
		return p_this;
	}
	lazy_add( NULL, sizeof(U44), s );
	U44* p_u44 = (U44*)p_alloc;
	n = s/sizeof(*p_u44);
...
gpcLAZY* gpcLAZY::tree_add8( U8 id, U8& n )
{
	...
	if( !this )
	{
		gpcLAZY* p_this = lazy_add( NULL, sizeof(U84), s, 8 );
		if( !p_this )
			return NULL;

		n = 1;
		((U84*)p_this->p_alloc)->null().x = id;
		return p_this;
	}
	lazy_add( NULL, sizeof(U84), s );
	U84* p_u84 = (U84*)p_alloc;
	n = s/sizeof(*p_u84);
