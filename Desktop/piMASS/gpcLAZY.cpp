...
gpcLAZY* gpcLAZY::lzyFRMT( U8& n_start, const char* p_format, ... )
{
	va_list vl;
...
gpcLAZY* gpcLAZY::tree_add( U4 id, U4& n )
{
	...
	if( !this )
	{
		gpcLAZY* p_this = lzyADD( NULL, sizeof(U4x4), s, 8 );
		if( !p_this )
			return NULL;
...
gpcLAZY* gpcLAZY::tree_add( U8 id, U8& n )
{
	...
	if( !this )
	{
		gpcLAZY* p_this = lzyADD( NULL, sizeof(U8x4), s, 8 );
		if( !p_this )
			return NULL;
...
gpcLAZY* gpcLAZY::tree_add( I8 id, I8& n )
{
	...
	if( !this )
	{
		gpcLAZY* p_this = lzyADD( NULL, sizeof(I8x4), s = -1, 8 );
		if( !p_this )
			return NULL;
