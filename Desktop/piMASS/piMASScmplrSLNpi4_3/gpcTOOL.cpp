...
U1x4* gpcPIC::TOOLexplode(	gpcLZYall& MANus, gpcPIC** ppPIC,
	...
	if( nB ) {
		...
		if( k )
		{
            ...
            for( U4 i = gpmMIN(nBOB+k2,nBOBall)-1; k2 <= i; i-- )
            {
				// copyback
				gpmDEL(ppBOB[i]);
				ppBOB[i] = ppBOB[i-k2];
...
U1x4* gpcPIC::TOOLmaskAB(	gpMEM* pMEM,
	...
	((gpcPIC*)NULL)->pRTXnx( pR, 0, 0 );
	gpc3Drtx* pRTX = pR->pRTXX;
	if( !pRTX )
		return NULL;
	int w = pRTX->xyWH.xyWH.w,
		h = pRTX->xyWH.xyWH.h;
...
U1x4* gpcPIC::TOOLmaskAB2(	gpMEM* pMEM,
							...
							char* pNAME, char *pPATH, char *pFILE ) {
	gpc3Drtx* pRTX = ((gpcPIC*)NULL)->pRTXnx( pR, 0, 0 );
	int w = pRTX->xyWH.xyWH.w,
		h = pRTX->xyWH.xyWH.h;
