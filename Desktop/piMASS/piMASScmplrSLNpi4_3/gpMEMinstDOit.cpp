...
I4 gpMEM::instDOit( gpOBJ& obj, U1* pU1 ) {
	...
	if((obj.AN.alf == gpeALF_TRGCP)||(obj.AN.alf >= gpeALF_TRGCPA && obj.AN.alf <= gpeALF_TRGCPZ)) {
		...
		SDL_Surface* pT = gpdGLpTRG->pRTXsrf(), *pS = gpdGLapPIC[iPIC]->pSRF;
		if( pS )
		if( pS->pitch*pS->h != pT->pitch*pT->h ) {
			gpmSDL_FreeSRF( gpdGLapPIC[iPIC]->pSRF );
			pS = NULL;
		}
		if( !pS )
			pS = gpdGLapPIC[iPIC]->pSRF = SDL_CreateRGBSurface( 0, pT->w, pT->h, 32, 0,0,0,0 );

		if( !pS )
			return cID;
		gpmMcpy( pS->pixels, pT->pixels, pS->pitch*pS->h );
		return cID;
	}
	/// GLSL SET TETURE -------------------------------------------------------
...
/*		case gpeALF_TRGCP: {								cID = gpeCsz_L; if( bCID ) break;
		...
		case gpeALF_MSKAB: {							cID = gpeCsz_L; if( bCID ) break;
				...
				if(mskID<1)
					break;
				if( gpdGLpTRG ? !gpdGLpTRG->pRTXX : true )
					break;
