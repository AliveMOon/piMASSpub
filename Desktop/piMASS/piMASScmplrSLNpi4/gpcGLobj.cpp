...
public:
	...
	gpc3Dmap(	U4 ix, gpcLZY& p, gpc3Dfc& fc,		// ix, pnt, fc,
				gpcLZY& b,							// mapBLwg,
				gpcLZY& t, U4x2 *pIX2,				// mapF2tx, pIX2,
				gpcLZY& ps,							// mapIXprt,
				void* p_u, void* p_nx				// pUi, pUnx
			) {
		gpmCLR;
		...
		while( pUi < pUnx ) {
			...
			pFl = fcl.pU4x2( uf );
			pFi = fci.pU4( (pFl->sum()-1)+pIX2->i ) - (pFl->n-1);
	...
	gpc3Dtri* pTRI0( gpcLZY& buf, const void* pLWO = NULL, U4x2* pTG = NULL ) {
		...
		for( U4 i = 0, p = -1, s = -1,nf, nt = 0; i < nFC; i++ ) {
			if( p != pPRT[i].prt ) {
				pTR = (gpc3Dtri*)tri.Ux( nt = tri.nLD(sizeof(*pTR)), sizeof(*pTR) );
				pTR->prt = p = pPRT[i].prt;

				if( nt )
				if( pTR ) {
					pSRF = pTR[-1].pSRFadd(-1);
					if( pTG )
					if( bSTDcout_3Df )
			...
			if( s != pPRT[i].srf ){
				...
				if( pSRF ){
					pSRF = pTR->pSRFadd(s);
					if( pTG ) if( bSTDcout_3Df )  gpdCOUT << " SRF:"	<< " " << ((char*)pLWO) + pTG[pSRF[-1].x].x
												<< " 1n:" << (pSRF[0].y-pSRF[-1].y)
												<< " 2n:" << (pSRF[0].z-pSRF[-1].z)/2
												<< " 3n:" << (pSRF[0].w-pSRF[-1].w)/3 << gpdENDL;
				} else
					pSRF = pTR->pSRFadd(s);
				if( pTG ) if( bSTDcout_3Df )  gpdCOUT << " SRF:"	<< " " << ((char*)pLWO) + pTG[pSRF->x].x
												<< " 1p:" << pSRF->y
												<< " 2p:" << pSRF->z/2
...
public:
	...
	gpc3Dly* SRFswp( const void* pV, U4 n ) {
		...
		while( pUi < pUnx ) {
			...
			prSUM.pU4n(srf.srf)[0]++;

			pPRT = mapIXprt.pU4x2n( srf.i );
			pPRT->srf = srf.srf;
			// FACE
			pFl = fc.LST.pU4x2n( srf.i );
			pFi = fc.IX.pU4n( pFl->sum()-1 ) - (pFl->n-1);

			for( U4 i = 0; i < pFl->n; i++ ) {
				pBl = (gpc3Dblnd*)mapBLwg.Ux( pFi[i], sizeof(*pBl) );
				pBl->SRFset( srf.srf+1 );
			}
			nF++;
		}
		return this;
	...
	gpc3Dly* SRFcout( gpcLZY& lzySRF, const void* pLWO, U4x2* pTG, U4 nGD = 1 ) {
		U1* pU0 = (U1*)pLWO;
		for( U4 i = 0, e = srSUM.nU4(), n; i < e; i++ ){
			n = *srSUM.pU4(i);
			if( n < nGD )
				continue;
			...
			if( bSTDcout_3Df )  gpdCOUT << i << " " << n << " " << pSRF->sNAME <<  gpdENDL;
		}
		return this;
	}
	...
	gpc3Dly* PRTswp( const void* pV, U4 n ) {
		...
		while( pUi < pUnx ) {
			...
			prSUM.pU4n(prt.prt)[0]++;

			pPRT = mapIXprt.pU4x2n( prt.i );
			pPRT->i = prt.i;
			pPRT->prt = prt.prt;

			pFl = fc.LST.pU4x2n( prt.i );
			pFi = fc.IX.pU4n( pFl->sum()-1 ) - (pFl->n-1);

			for( U4 i = 0; i < pFl->n; i++ ) {
				pBl = (gpc3Dblnd*)mapBLwg.Ux( pFi[i], sizeof(*pBl) );
				pBl->PRTset( prt.prt+1 );
			}
	...
	gpc3Dly* PRTcout( const void* pLWO, U4x2* pTG, U4 nGD = 1 ) {
		U1* pU0 = (U1*)pLWO;
		for( U4 i = 0, e = prSUM.nU4(), n; i < e; i++ ){
			n = *(prSUM.pU4(i));
			if( n < nGD )
				continue;
			if( bSTDcout_3Df )
				gpdCOUT << i << " " << n << " " << (char*)(pU0+pTG[i].x) <<  gpdENDL;
		}
		return this;
	}
...
gpc3Dgym* gpc3Dgym::pLWS( gpcLZY& lws, gpcLZYdct& lwsDCT, gpcLZYdct& bnDCT ) {
	if( !this )
		return NULL;
	if( bSTDcout_3Df )
		gpdCOUT << "gpc3Dgym::pLWS " << sPATH << gpdENDL;
	...
	for( pS += gpmNINCS( pS, " \t\r\n" ); pS < pSe; pS += gpmNINCS( pS, " \t\r\n" ) ) {
		...
		switch( si ) {
			...
			case gpeLWScom_C_close: {
				break;
				pIS = gpdITMis;
				int nK = pIS->cnl.nKEY(iCNL);
				if( pIS )
				if( bSTDcout_3Df )  gpdCOUT << " " << nENV << "/" << nK << " c:" << iCNL << gpdFLUSH;
			} break;
...
gpc3D* gpc3D::pLWO( gpcLZY& lwo, gpcLZYdct& bnDCT ) {
	...
	if( bSTDcout_3Df )  gpdCOUT << " i8w=" << i8w << ": "  << n << "-" <<  p_lwo->nLD() << "=" <<  p_lwo->nLD()-n << gpdENDL; //endl;
	...
	while( pUnx < pUe ) {
		...
		pUi = (char*)pU4i;
		pUnx = pUi+n;
		if( bSTDcout_3Df )  gpdCOUT << pUi-pU0 << " LWO_ID_"<< (char*)chunk << ": "  << gpdFLUSH; //endl;
		switch( chunk[0] ) {
			case LWO_ID_TAGS: {
					while( pUi < pUnx ) {
			...
			case LWO_ID_LAYR: {
					if( pLY ) {
						if( !pMAD )
							pMAD = pLY->pMADswp( NULL, 0 );

						if( pMAD )
						if( bSTDcout_3Df )
							gpdCOUT << pLY->mapDCTtx.sSTRix(pMAD->id,"ERR")
									<< " nP:"	<< pLY->pnt.nLD(sizeof(F4))
									<< " nPm:"	<< pMAD->pnt.nLD(sizeof(gpc3Dvx)) << gpdENDL;
						pLY = NULL;
					}
					pMAD = NULL;
			...
			case LWO_ID_POLS: {
					...
					switch( *pU4i ) {
						case LWO_ID_FACE: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_FACE:"<<  gpdENDL;
								pLY->fc.pFACEswp(pUi,pUnx-pUi)->COUT();
							} break;
						case LWO_ID_BONE: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_BONE:"<<  gpdENDL;
							} break;
						default: break;
					}
				} break;
			case LWO_ID_VMAP: {
					...
					switch( *pU4i ) {
						...
						case LWO_ID_WGHT: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_WGHT:"<<  gpdFLUSH;
								I4 ix = pLY->pWGHTswp( pUi, pUnx-pUi, bnDCT );
								U4x2 *pIX2 = (U4x2*)pLY->mapIXwg.Ux( ix, sizeof(*pIX2) );
								if( bSTDcout_3Df )  gpdCOUT << pLY->mapDCTwg.sSTRix(ix,"ERR")  << " iBx:" << pIX2->x << " n:" << pIX2->y << gpdENDL;
							} break;
						case LWO_ID_TXUV: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_TXUV:"<<  gpdFLUSH;
								I4 ix = pLY->pTXUVswp( pUi, pUnx-pUi );
								U4x2 *pIX2 = (U4x2*)pLY->mapIXtx.Ux( ix, sizeof(*pIX2) );
								if( bSTDcout_3Df )  gpdCOUT << pLY->mapDCTtx.sSTRix(ix,"ERR") << " iTX:" << pIX2->x << " nTX:" << pIX2->y << gpdENDL;
							} break;
						case LWO_ID_MORF: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_MORF:"<<  gpdFLUSH;
								ix = pLY->mapDCTmr.nIX();
								nUi = gpmSTRLEN(pUi);
			...
			case LWO_ID_VMAD: {
					...
					switch( *pU4i ) {
						...
						case LWO_ID_TXUV: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_TXUV:"<<  gpdENDL;
								pMAD = pLY->pMADswp( pUi, pUnx-pUi );
							} break;
						case LWO_ID_MORF: {
							} break;
						default: break;
			...
			case LWO_ID_PTAG: {
					...
					switch( *pU4i )
					{
						case LWO_ID_COLR:
							break;
						case LWO_ID_SURF: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_SURF:"<<  gpdENDL;
								pLY->SRFswp( pUi, pUnx-pUi )->SRFcout( lzySRF, pU0, pTG );
							} break;
						case LWO_ID_PART: {
								if( bSTDcout_3Df )  gpdCOUT << "\tLWO_ID_PART:"<<  gpdENDL;
			...
			case LWO_ID_CLIP: {
					...
					if( bSTDcout_3Df )  gpdCOUT << pLY->mapDCTcl.sSTRix(ix,"ERR") << gpdENDL;
				} break;
			case LWO_ID_SURF: {
					nUi = gpmSTRLEN(pUi);
					...
					U4 nU4 = *(U4*)(pLY->srSUM.Ux(ix,sizeof(U4)));
					if( bSTDcout_3Df )  gpdCOUT << ix << " " << iS << " "<< nU4 << " " << pSRF->sNAME <<  gpdENDL;
					...
					while( pUi < pUnx ) {
						...
						pSUB = pUi;
						pUi += n;
						if( bSTDcout_3Df )  gpdCOUT << (char*)chunk << " "<< n << gpdENDL;
						switch( chunk[0] ) {
							case LWO_ID_BLOK: {
									while( pSUB < pUi ) {
										chunk[0] = *(U4*)pSUB; pSUB += sizeof(U4);
										n = swp2( pSUB ); pSUB += sizeof(U2);
										if( bSTDcout_3Df )  gpdCOUT << " " << (char*)chunk << " " << n << gpdFLUSH;
										pSS = pSUB;
										pSUB += n;
										switch( chunk[0] ) {
											case LWO_ID_IMAG: {
													...
													if( pPNT )
														sprintf( pPNT, ".png" );
													if( bSTDcout_3Df )  gpdCOUT << " " << pSRF->imag << " " << pSRF->sPIC <<  gpdENDL;
												} break;
											case LWO_ID_VMAP: {
													gpmSTRCPY( pSRF->sVMAP, pSS );
													pSRF->imap = pLY->mapDCTtx.dctFND( pSRF->sVMAP, gpmSTRLEN(pSRF->sVMAP), n );
													if( bSTDcout_3Df )  gpdCOUT << " " << pSRF->imap << " " << pLY->mapDCTtx.sSTRix(pSRF->imap,"ERR") <<  gpdENDL;
												} break;
					...
					if( bSTDcout_3Df )  gpdCOUT << gpdENDL;
				} break;
			default: break;
		}
...
I4x4 gpcGL::VBOobj( I4x4& iVXn, const gpc3Dvx* pVX, U4 nD ) {
	//Create VBO
	if( iVXn.y )
		return iVXn;
	iVXn.y++;
...
I4x4 gpcGL::IBOobj( I4x4& iIXn,const GLuint* pD, U4 nD, U4 nX ) {
	iIXn.a4x2[1] = U4x2( nX, nD );
	if( !iIXn.a4x2[1].area() ) {
		if( iIXn.x ) {
			glDeleteBuffers( 1, (GLuint*)&iIXn.x );
			iIXn.x = -1;
		}
		iIXn.y = 0;
		return iIXn;
...
gpcGL* gpcGL::glDRW3D( U4 l, gpc3D* p3D, U8 msk ) {
	...
	/// VERTEX ------------------------------------------
			glBindBuffer( GL_ARRAY_BUFFER, pM->iVXn.x );  gpfGLerr( "GL_ARRAY_BUFFER 1" );
			onATscn(aglVXi4);
			...
			for( U4 t = 0; t < nT; t++, msk >>= 1 ) {
				...
				if( !pSRFx4 )
					continue;
	/// ELEMENT ------------------------------------------
				U4	*pU4 = pT0[t].t.pU4(), nU4 = pT0[t].t.nU4(3), nCHK = 0;
				glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, pT0[t].aIIXN[2].x );  gpfGLerr( "GL_ELEMENT_ARRAY_BUFFER");
				for( U4 s = 0, ns = pT0[t].nSRF(); s < ns; s++ ) {
					aglVXi4[0x7] = pSRFx4[s].w;				// strt 	const void * indices
					aglVXi4[0x8] = pSRFx4[s+1].w;			// end
					aglVXi4[0x9] = aglVXi4[8]-aglVXi4[7];	// cnt			GLsizei count,
					aglVXi4[0xa] = aglVXi4[0x9]/3;			// cnt/3 nTRI
					if( aglVXi4[0xa] < 1 )
						continue;
					aglVXi4[0xb] = aglVXi4[0x7]*sizeof(U4); // const void * indices
					pPIC = pPICsrf( p3D->pSRFi(pSRFx4[s].x), p3D->sPATH );
					if( !pPIC )
						continue;
					nCHK += aglVXi4[0xa];
					glSETtx( 0, pPIC->surDRWtx(pRNDR), pPIC->txWH.a4x2[1] );
					glDrawElements( gpaDRWmod[3],				// GLenum mode,
									aglVXi4[0x9],				// GLsizei count,
									GL_UNSIGNED_INT,			// GLenum type,
									(void*)aglVXi4[0xb] );		// const void * indices
				}
				glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, 0 ); gpfGLerr();

				if( bSTDcout_3Df )
					gpdCOUT << (int)nCHK<< "/"<< (int)nU4 << gpdENDL;
			}

			offATscn();

			glBindBuffer( GL_ARRAY_BUFFER, 0 ); gpfGLerr( "GL_ARRAY_BUFFER 0" );
...
"	vec3	vmx = (nBON.x<1) ? v_vx : (aMX[ix]*aMXi[ix]*vec4(v_vx,1.0)).xyz,		\n"
	...
	*sPUB = 0;
	...
	for( U4 iAT = pAT[0].aALFvan( alfSCN0, nAT, gpmN(alfSCN0) ), iAnx, iAT1;
		...
		*sPUB = 0;
		...
		switch( pAT[iAT].alf ) {
			case gpeALF_CAM:
						if( bSTDcout_3Df )  gpdCOUT <<  (apI[0] ? apI[0]->ID : (I8)-1) << " " <<  (apI[1] ? apI[1]->ID : (I8)-1) << " " << gpdENDL;
    ...
    if( *sPUB )
	if( bSTDcout_3Df )  gpdCOUT << sPUB << gpdENDL;
